...
gdata2 <- data.frame(
  ...
  y=-log10(res2$padj),
  name_ =  "Breast tumor vs Normal breast tissue"
)

gdata3 <- rbind(gdata, gdata2)
res3 <- rbind(res, res2)

gdata3 <- gdata3 %>%
  mutate(color = ifelse(res3$padj > 0.01, 
                        yes = "Significant", 
                        no =  "Not Significant"))



ggplot(data=gdata3, aes(x=x, y=y)) +
...
#Визуализация пасвея
res_ord <- res[order(res$stat), ]
to_visualise2 <- counts.norm[rownames(res_ord), order(conditions[, 2])]
gen_names <- intersect(rownames(to_visualise2), genesToVisualise)
to_visualise3 <- to_visualise2[gen_names,]
to_visualise4 <- t(apply(to_visualise3, 1, function(r) {
  (r - min(r)) / (max(r) - min(r))
}))



pheatmap(to_visualise4, 
         show_rownames = F, cluster_rows = F,
         cluster_cols=F,
         annotation_col = conditions)
# dev.off()
...
draw.pairwise.venn(length(rownames(resone)), length(rownames(restwo)), length(commongenes),
                   category = c("Treg vs Tconv", "Tumor tissue VS normal tissue"),
                   fill            = c("#59d5ff", "#ffdbe2"),
                   lty             = "blank",
                   ext.pos         = 15,
                   ext.line.lwd    = 2,
                   ext.line.lty    = "dashed")
